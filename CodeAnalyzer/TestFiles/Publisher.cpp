...
void Publisher::generateHTML(std::unordered_map<std::string, std::vector<std::string>> DepTable, std::unordered_map <std::string, std::vector<std::pair<int, int>>> table)
{
	std::ofstream contentHTML; //Adding index.html, a file that contains path to all html files published
	contentHTML.open("./OutputFiles/index.html");
	contentHTML << htmlTag();
	contentHTML << headTag();
	contentHTML << "<ul>Pages created</ul>";
	std::cout << "\n Requirement 5 & 6: CSS \"stylesheet.css\" and JavaScript \"bonustoggle.js\" included in each page's head \n";
	std::cout << " ---------------------------------------------------------------------------------------------------------- \n";
	std::cout << "\n Requirement 7: Dependent files are shown as links \n";
	std::cout << " --------------------------------------------------- \n";
	std::cout << "\n Requirement 9: Demonstrating Publisher by publishing important packages \n";
	std::cout << "\n ----------------------------------------------------------------------- \n";
	for (std::unordered_map<std::string, std::vector<std::string>>::const_iterator iter = DepTable.begin(); iter != DepTable.end(); ++iter)
	{
		...
		std::ofstream html;
		std::string filename = "./OutputFiles/"+ExtractFilename(iter->first);
	...
	std::cout << "\n Requirement 4: Classes and Functions can be Expanded or collapsed by clicking { in web pages \n";
	std::cout << " ---------------------------------------------------------------------------------------------- \n";
}
