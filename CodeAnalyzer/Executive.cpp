...
using Rslt1 = Logging::StaticLogger<0>;  // use for application results
using Demo1 = Logging::StaticLogger<1>;  // use for Demo1nstrations of processing
using Dbug1 = Logging::StaticLogger<2>;  // use for debug output
...
void showUsage()
{
	...
	out << "\n    - d : set logger to display Demo1 outputs";
...
//<!----processes cpp header to get package name
void CodeAnalysisExecutive::processcppHeader(bool showProc) {
	for (auto file : cppHeaderFiles()) {
		...
		Rslt1::flush(); Demo1::flush(); Dbug1::flush();
		if (!Rslt1::running())
			Demo1::write("\n\n  opening file \"" + pRepo_->package() + "\"");
		if (!Demo1::running() && !Rslt1::running())
			Dbug1::write("\n\n  opening file \"" + pRepo_->package() + "\"");
		pRepo_->language() = Language::Cpp;
...
//Process cpp Implemenation and gets package to be added into SLOC map
void CodeAnalysisExecutive::processcppImp(bool showProc) {
...
//Simple function for returning hyphens
std::string CodeAnalysis::CodeAnalysisExecutive::putHyphens()
{
	return "------------------------------------------------------------------------------------------------------------------\n";
}
...
{
	processcppHeader(showProc);
	processcppImp(showProc);
...
void CodeAnalysisExecutive::startLogger(std::ostream& out)
{
	...
	// will start Demo1 and Dbug1 if  have options /d and /b
...
void CodeAnalysisExecutive::setLogFile(const File& file)
{
	...
	if (pLogStrm_->good())
	{
		Rslt1::attach(pLogStrm_);
		Demo1::attach(pLogStrm_);
		Dbug1::attach(pLogStrm_);
	}
	else
		Rslt1::write("\n  couldn't open logFile.txt for writing");
}
//Get the current path of executing exe file
std::string CodeAnalysisExecutive::ExePath() {
...
//Creates a map of all functions, class, globalfunction, interface, lambda and their size
std::unordered_map <std::string, std::vector<std::pair<int, int>>> CodeAnalysisExecutive::getLineTable(ASTNode* root)
{
	std::unordered_map <std::string, std::vector<std::pair<int, int>>> lineTable;
	std::function<void(ASTNode* pNode)> co = [&](ASTNode* pNode) {
		if (
			pNode->type_ == "class" ||
			pNode->type_ == "function" ||
			pNode->type_ == "globalfunction" ||
			pNode->type_ == "interface" ||
			pNode->type_ == "lambda"
			)
			fileNodes_.push_back(std::pair<File, ASTNode*>(pNode->package_, pNode));
	...
	for (auto item : fileNodes_)
	{
		//std::cout << item.first << " " << item.second->startLineCount_ << " " << item.second->endLineCount_ << " " <<item.second->name_<<" "<< item.second->type_ << std::endl;
		lineTable[item.first].push_back(std::make_pair(item.second->startLineCount_, item.second->endLineCount_));
	}
	//out << std::setw(25) << trunc(file, 23);
	return lineTable;
}
//Function to get globalscope ASTNode which will be used to generate line table (map) by calling getLineTable
std::unordered_map <std::string, std::vector<std::pair<int, int>>> CodeAnalysis::CodeAnalysisExecutive::getnamespace()
{
	ASTNode* pGlobalScope = pRepo_->getGlobalScope();
	//ASTNode* pNode = ASTref_.root();
	std::unordered_map <std::string, std::vector<std::pair<int, int>>> table = getLineTable(pGlobalScope);
	return table;
}
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		std::cout << "\n Requirement 8: Command line takes files to publish, file patterns and HTML file to open in firefox as arguments \n" << exec.putHyphens();
		exec.getSourceFiles();
		...
		std::unordered_map<std::string, std::vector<std::string>> db = da.findDependency(newtable, vec);
		std::cout << "\n\n Requirement 3: Creating Web Pages by capturing C++ source code file \n" << exec.putHyphens();
		std::unordered_map <std::string, std::vector<std::pair<int, int>>> tab=exec.getnamespace();
		Publisher pr;
		pr.generateHTML(db,tab,"./OutputFiles/");
		std::string htmlarg = "-a";
		std::string htmlFile = "./OutputFiles/";
		if (argv[argc - 2] == htmlarg && argv[argc - 1] != nullptr)
		{
			htmlFile += argv[argc - 1];
			if (CheckFile(htmlFile.c_str())==true)
				pr.OpenInBrowser(htmlFile,"chrome");
			else
				std::cout << "\n File not found, please input proper file name in command line \n";
		}
	}
	...
	std::cout << "\n Requirement 10: All functions called by this automated test suite \n";
	std::cout << " ----------------------------------------------------------------- \n";
	system("PAUSE");
	return 0;
}
